...
class Group {
  static #groups = {}; // Створюємо приватне статичне поле #groups використовується для зберігання усіх створених груп. Має початкове значени: пустий об'єкт
  ...
  static create(name) {
    ...
    if (!this.#groups[name]) {
      this.#groups[name] = new Group(name); // то вона створюється та зберігається в полі #groups.
    }
    return this.#groups[name]; // в кінці повертає #groups[name]
  }
}
